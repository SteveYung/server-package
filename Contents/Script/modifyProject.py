# 2015.01.17 10:32:32 CST
#Embedded file name: /Projects/GitLab/rsdk_package/Env/Script/modifyProject.py
import datetime
import json
import ntpath
import os
...
FILE_TYPE_INFO = {'archive.ar': 'PBXFrameworksBuildPhase',
 ...
 'sourcecode.text-based-dylib-definition':'PBXFrameworksBuildPhase'}
...
class XCBuildConfiguration(PBXType):
    ...
    def add_search_paths(self, paths, base, key, recursive = True, escape = True):
        ...
        for path in paths:
            if path.find('Applications/Xcode.app/Contents/Developer/Platforms') != -1:
                continue
            if recursive and not path.endswith('/**'):
                path = os.path.join(path, '**')
            if os.path.isabs(path):
                add_path = path
            elif path[0:11] == '$(SRCROOT)/' or path[0:12] == '$(inherited)':
                add_path = path
            else:
    ...
    def modify_bundle_identifier(self, bundleid,targetname):
        modified = False
        base = 'buildSettings'
        ...
        if isinstance(bundleid, basestring):
            bundleid = PBXList(bundleid)
        if base not in self:
            self[base] = PBXDict()
        #check target. it can't
        if self[base].has_key(productnamekey) and self[base][productnamekey] == targetname:
            self[base][key] = bundleid
            modified = True
        
        #7.7 for xcode7 build setting
        for flag in bundleid:
            if key not in self[base]:
                self[base][key] = PBXList()
            elif isinstance(self[base][key], basestring):
                self[base][key] = PBXList(self[base][key])
    ...
    #7.11 show buildsetting search path
    def showSearchPathInfo(self):
        base = 'buildSettings'
        key = 'HEADER_SEARCH_PATHS'
        print '****************************************************************************'
        print '******************BUILD SETTINGS SEARCH PATH******************'
        print '****************************************************************************'
        for e in  PBXList(self[base][key]):
            print '****         ' + e
        print '****'
        print '****************************************************************************' + '\n'
        ...
        key2 = 'OTHER_LDFLAGS'
        print '****************************************************************************'
        print '*************BUILD SETTINGS OTHER_LINK_FLAGS***************'
        print '****************************************************************************'
        for e in  PBXList(self[base][key2]):
            print '****         ' + e
        print '****************************************************************************'
    ...
    def remove_other_ldflags(self, flags):
        ...
        return modified
    #5.30 add_codesign_entitlement fun
    def add_codesign_entitlement(self, flags):
        modified = False
        base = 'buildSettings'
    ...
    def modify_validarchs(self, finded):
        modified = False
        base = 'buildSettings'
        key = 'VALID_ARCHS'
        if base in self:
            if key not in self[base]:
                if finded == False:
                    self[base][key] = PBXList()
                    if self[base][key].add('armv7 armv7s'):
                        self[base][key] = [ e for e in self[base][key] if e ]
                        modified = True
            else:
                if isinstance(self[base][key], basestring):
                    self[base][key] = PBXList(self[base][key].split(' '))
                if self[base][key].remove('arm64'):
                    self[base][key] = [ e for e in self[base][key] if e ]
                    modified = True
        return modified

    def get_infoplistfile(self):
        base = 'buildSettings'
        key = 'INFOPLIST_FILE'
        if base in self:
            if key in self[base]:
...
class XcodeProject(PBXDict):
    ...
    def modify_bundle_identifier(self, bid):
        build_configs = [ b for b in self.objects.values() if b.get('isa') == 'XCBuildConfiguration' ]
        targetname=self.get_target_name()
        for b in build_configs:
            if b.modify_bundle_identifier(bid,targetname):
                self.modified = True
    ...
    def add_codesign_entitlement(self, flags):
        build_configs = [ b for b in self.objects.values() if b.get('isa') == 'XCBuildConfiguration' ]
        for b in build_configs:
            if b.add_codesign_entitlement(flags):
                self.modified = True
    #7.11 show buildsetting search path
    def showSearchPathInfo(self):
        build_configs = [ b for b in self.objects.values() if b.get('isa') == 'XCBuildConfiguration' ]
        for b in build_configs:
            b.showSearchPathInfo()
            break
    ...
    def get_infoplistfile(self):
        ...
        arr = []
        for b in build_configs:
            if b.get_infoplistfile() != None:
                find = False
                for arrchild in arr:
                    if b.get_infoplistfile() == arrchild:
                        find = True

                if find == False:
                    arr.append(b.get_infoplistfile())

        infofile = ''
        #this function only get the "last" targetname
        #maybe we can fix this problem near future
        targetname = self.get_target_name()
        for arrchild in arr:
            #for xcode 5 info.plist
            if arrchild == targetname + '/Info.plist':
                return arrchild
            #for xcode 6 & above info.plist
            if targetname+'-Info.plist' in arrchild:
                return arrchild.replace('$(SRCROOT)/','')
            #for first target info.plist--default
            if len(arrchild) < len(infofile) or len(infofile) == 0:
                infofile = arrchild

        return infofile

    def modify_validarchs(self):
        build_configs = [ b for b in self.objects.values() if b.get('isa') == 'XCBuildConfiguration' ]
        finded = False
        for b in build_configs:
            if b.find_validarchs():
                finded = True

        for b in build_configs:
            if b.modify_validarchs(finded):
                self.modified = True
    ...
    def get_files_by_os_path(self, os_path, tree = 'SOURCE_ROOT'):
        files = [ f for f in self.objects.values() if f.get('isa') == 'PBXFileReference' and f.get('path') == os_path and f.get('sourceTree') == tree ]
        return files

    def get_files_by_name(self, name, parent = None):
        if parent:
            files = [ f for f in self.objects.values() if f.get('isa') == 'PBXFileReference' and f.get('name') == name and parent.has_child(f) ]
        else:
    ...
    def get_groups_by_os_path(self, path):
        ...
        groups = [ g for g in self.objects.values() if g.get('isa') == 'PBXGroup' and os.path.abspath(g.get('path', '/dev/null')) == path ]
        return groups
    ...
    def get_target_name(self):
        ...
        for obj in self.objects.values():
            if obj.get('isa') == 'PBXNativeTarget':
                arr.append(obj.get('name'))
                if i > 0:
                    for j in range(0, i):
                        if arr[i] == arr[j] + 'Tests':
                            return arr[j]
                        if arr[i] + 'Tests' == arr[j]:
                            return arr[i]
    ...
    def get_configurations(self):
        for obj in self.objects.values():
            if obj.get('isa') == 'XCConfigurationList' and obj.get('defaultConfigurationName') != None:
                return obj.get('defaultConfigurationName')
        ...
        for grp_path, subdirs, files in os.walk(os_path):
            ...
            for f in files:
                if f[0] == '.' or [ m for m in excludes if re.match(m, f) ]:
                    continue
                kwds = {'create_build_files': create_build_files,
                 'parent': grp,
                 'name': f}
    ...
    def apply_mods(self, mod_dict, default_path = None):
        ...
        for k, v in mod_dict.items():
            ...
            #5.30 add_codesign_entitlement fun
            elif k == 'CODE_SIGN_ENTITLEMENTS':
                self.add_codesign_entitlement(v)
        ...
        if compiler_flags:
            for k, v in compiler_flags.items():
                filerefs = []
                for f in v:
                    filerefs.extend([ fr.id for fr in self.objects.values() if fr.get('isa') == 'PBXFileReference' and fr.get('name') == f ])

                buildfiles = [ bf for bf in self.objects.values() if bf.get('isa') == 'PBXBuildFile' and bf.get('fileRef') in filerefs ]
    ...
    def Load(cls, path):
        if os_is_mac():
            ...
            exit(1)
        p = subprocess.Popen([XcodeProject.plutil_path,
         '-convert',
         'xml1',
         '-o',
         '-',
...
# +++ okay decompyling rsdk1.4/Script/modifyProject.pyc
